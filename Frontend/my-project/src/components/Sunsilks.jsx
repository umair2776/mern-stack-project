...
const Sunsilks = () => {
  ...
  const fetchData = async () => {
    const response = await axios.get(
      "http://localhost:3000/api/admin/product/get?category=sunsilk"
    );
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4 md:px-8">
      {product.map((product, index) => (
        <Link to={`/product/${product._id}`} key={index}>
          <div className="bg-white mt-8 mb-8 w-full hover:shadow-xl rounded-lg p-4 transition-transform duration-300 transform hover:scale-105">
